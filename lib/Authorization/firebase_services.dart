...
static Future<void> toggleChatArchive(String chatId, bool isArchived) async {
  try {
    ...
    developer.log('✅ Chat ${isArchived ? 'archived' : 'unarchived'}: $chatId');
...
static Future<DocumentReference> sendMessage({
  ...
  try {
    ...
    developer.log('Receiver FCM token exists: ${fcmToken != null && fcmToken.toString().isNotEmpty}');
    ...
    await _updateChatDocument(chatId, senderId, receiverId, message, senderName, messageRef.id);
...
static Future<bool> _sendNotificationHTTPv1({
  ...
  static Future<void> toggleNotifications(String userId, bool enabled) async {
    try {
      ...
      developer.log('Notifications ${enabled ? 'enabled' : 'disabled'} for user: $userId');
    } catch (e) {
  ...
  static Future<QuerySnapshot> searchUsers(String query) async {
    ...
    try {
      var results = await _firestore
          ...
          .where('nameSearchable', isLessThanOrEqualTo: queryLower + '\uf8ff')
          .limit(20)
          .get();
      ...
      if (results.docs.isEmpty) {
        results = await _firestore
            ...
            .where('emailSearchable', isLessThanOrEqualTo: queryLower + '\uf8ff')
            .limit(20)
            .get();
      }
      
      developer.log('Search for "$query" returned ${results.docs.length} results');
      return results;
    } catch (e) {
